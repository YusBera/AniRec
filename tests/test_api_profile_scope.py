...
def _activate_local_profile(app, username):
...
def test_feedback_cannot_write_another_profile(tmp_path, action):
    ...
    active = _activate_local_profile(app, "active")
    other = _activate_local_profile(app, "other")
    app.state.container.profiles.set_active(active.profile_id)
...
def test_operation_handler_keeps_captured_profile_and_token_after_switch(tmp_path, monkeypatch):
    ...
    original = _activate_local_profile(app, "original")
    other = _activate_local_profile(app, "other")
    services.profiles.set_active(original.profile_id)
    ...
    services.profiles.set_active(other.profile_id)
